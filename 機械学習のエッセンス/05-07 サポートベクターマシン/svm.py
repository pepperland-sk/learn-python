...
class RBFKernel:
    ...
    def value(self, i, j): #引数iとjに対してカーネル関数(RBF:放射基底関数)を計算する
...
class SVC:
    ...
    def fit(self, X, y, selections=None):
        ...
        for _ in range(self.max_iter):
            s = a != 0.
            ydf = y*(1-np.dot(a[s]*y[s], kernel.eval(X, s)).T) #y∇f(a)tの計算を行う
            iydf = np.c_[indices, ydf]
            i = int(min(iydf[((a > 0) & (y > 0)) | ((a < self.C) & (y < 0))], key=itemgetter(1))[0]) #iの値をargminで求める(hardと異なる)
            j = int(max(iydf[((a > 0) & (y < 0)) | ((a < self.C) & (y > 0))], key=itemgetter(1))[0]) #jの値をargmaxで求める(hardと異なる)
            if ydf[i] >= ydf[j]:
                break
            ...
            ai = ((1 - y[i]*y[j] + y[i]*((kij - kjj)*ay2 - (a[s]*y[s]*(kxi-kxj)).sum())) / (kii + kjj - 2*kij)) #a^iの処理を行う
            if ai < 0:
                ai = 0
            elif ai > self.C:
                ai = self.C #a^i>Cとなる場合の考慮を追加
            aj = (-ai * y[i] - ay2) * y[j]

            if aj < 0:
                aj = 0
                ai = (-aj * y[j] - ay2) * y[i]
            elif aj > self.C: #a^j>Cとなる場合の考慮を追加
                aj = self.C
                ai = (-aj * y[j] - ay2) * y[i]
